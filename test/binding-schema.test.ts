...
function receipt(schemaVersion: 1 | 2 | 3, technicalStatus: string): Record<string, unknown> {
  return {
...
test("binding receipt schema ties technical status to the evidence schema version", async () => {
  ...
  assert.equal(validate(receipt(3, "qualified-self-recorded")), true, JSON.stringify(validate.errors));
  assert.equal(validate(receipt(3, "not-qualified-self-recorded")), true, JSON.stringify(validate.errors));
  assert.equal(validate(receipt(3, "not-recomputable")), false);
});
