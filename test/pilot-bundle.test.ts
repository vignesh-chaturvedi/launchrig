...
function createTestPackageArchive(
  ...
  const paths = [
    ...
    ...packedTemplates.map((entry) => "package/" + entry),
...
test("publisher bundle v15 rejects the RC16-only rehearsal claim", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v15-rehearsal-"));
  try {
    const archive = createTestPackageArchive([], BUNDLE_PROFILE_V15);
    const manifest = await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V15);
    manifest.consumerRehearsal.checks = [...V16_REHEARSAL_CHECKS];
    await writeFile(path.join(directory, "manifest.json"), JSON.stringify(manifest, null, 2) + "\n", "utf8");
    await assert.rejects(
      () => bundleVerifier.verifyPublisherBundle(directory),
      /consumer rehearsal contract is invalid/,
...
test("publisher bundle v14 rejects the RC15-only rehearsal claim", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v14-rehearsal-"));
  try {
    const archive = createTestPackageArchive([], BUNDLE_PROFILE_V14);
    const manifest = await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V14);
    manifest.consumerRehearsal.checks = [...V15_REHEARSAL_CHECKS];
    await writeFile(path.join(directory, "manifest.json"), JSON.stringify(manifest, null, 2) + "\n", "utf8");
    await assert.rejects(
      () => bundleVerifier.verifyPublisherBundle(directory),
      /consumer rehearsal contract is invalid/,
...
test("publisher bundle v12 rejects the RC13-only rehearsal claim", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v12-rehearsal-"));
  try {
    const archive = createTestPackageArchive([], BUNDLE_PROFILE_V12);
    const manifest = await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V12);
    manifest.consumerRehearsal.checks = [...V13_REHEARSAL_CHECKS];
    await writeFile(path.join(directory, "manifest.json"), JSON.stringify(manifest, null, 2) + "\n", "utf8");
    await assert.rejects(
      () => bundleVerifier.verifyPublisherBundle(directory),
      /consumer rehearsal contract is invalid/,
...
test("publisher bundle v11 rejects the RC12-only rehearsal claim", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v11-rehearsal-"));
  try {
    const archive = createTestPackageArchive([], BUNDLE_PROFILE_V11);
    const manifest = await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V11);
    manifest.consumerRehearsal.checks = [...V12_REHEARSAL_CHECKS];
    await writeFile(path.join(directory, "manifest.json"), JSON.stringify(manifest, null, 2) + "\n", "utf8");
    await assert.rejects(
      () => bundleVerifier.verifyPublisherBundle(directory),
      /consumer rehearsal contract is invalid/,
...
test("publisher bundle v12 requires its recruitment preparation inventory", async () => {
  for (const omittedPath of [
    ...PACKED_COMPILED_ADDITIONS_V12_ONLY,
    "package/docs/publisher-recruitment.md",
    "package/schemas/launchrig-private-cohort-register-draft-result.schema.json",
    "package/templates/publisher-fit-check.md",
  ]) {
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v12-required-"));
    try {
      const archive = createTestPackageArchive([], BUNDLE_PROFILE_V12, [omittedPath]);
      await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V12);
      await assert.rejects(
        () => bundleVerifier.verifyPublisherBundle(directory),
        /RC12 is missing|inventory does not match the release allowlist/,
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
...
test("publisher bundle v13 requires its prospect review confirmation-gate inventory", async () => {
  for (const omittedPath of [
    ...PACKED_COMPILED_ADDITIONS_V13_ONLY,
    "package/docs/publisher-prospect-review.md",
    "package/schemas/launchrig-private-prospect-review-result.schema.json",
    "package/schemas/launchrig-private-prospect-review.schema.json",
  ]) {
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v13-required-"));
    try {
      const archive = createTestPackageArchive([], BUNDLE_PROFILE_V13, [omittedPath]);
      await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V13);
      await assert.rejects(
        () => bundleVerifier.verifyPublisherBundle(directory),
        /RC13 is missing|inventory does not match the release allowlist/,
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
...
test("publisher bundle v15 requires its human send decision confirmation-gate inventory", async () => {
  for (const omittedPath of [
    ...PACKED_COMPILED_ADDITIONS_V15_ONLY,
    "package/docs/publisher-send-decision-recording.md",
    "package/schemas/launchrig-private-human-send-decision-result.schema.json",
    "package/schemas/launchrig-private-human-send-decision.schema.json",
  ]) {
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v15-required-"));
    try {
      const archive = createTestPackageArchive([], BUNDLE_PROFILE_V15, [omittedPath]);
      await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V15);
      await assert.rejects(
        () => bundleVerifier.verifyPublisherBundle(directory),
        /RC15 is missing|inventory does not match the release allowlist/,
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
...
test("publisher bundle v16 requires its structured configuration diagnostics inventory", async () => {
  for (const omittedPath of [
    ...PACKED_COMPILED_ADDITIONS_V16_ONLY,
    "package/schemas/launchrig-config-validation-result.schema.json",
    "package/schemas/fixtures/launchrig-config-rule-fixtures.v1.json",
  ]) {
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v16-required-"));
    try {
      const archive = createTestPackageArchive([], BUNDLE_PROFILE_V16, [omittedPath]);
...
test("publisher bundle v15 rejects the RC16-only configuration diagnostics inventory", async () => {
  for (const addedPath of [
    ...PACKED_COMPILED_ADDITIONS_V16_ONLY,
    "package/schemas/launchrig-config-validation-result.schema.json",
    "package/schemas/fixtures/launchrig-config-rule-fixtures.v1.json",
  ]) {
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v15-frozen-"));
    try {
      const archive = createTestPackageArchive([addedPath], BUNDLE_PROFILE_V15);
      await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V15);
      await assert.rejects(
        () => bundleVerifier.verifyPublisherBundle(directory),
        /outside the release allowlist|RC16 file outside its historical profile/,
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
...
test("publisher bundle v14 rejects the RC15-only human send decision inventory", async () => {
  for (const addedPath of [
    ...PACKED_COMPILED_ADDITIONS_V15_ONLY,
    "package/docs/publisher-send-decision-recording.md",
    "package/schemas/launchrig-private-human-send-decision-result.schema.json",
    ...
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v14-frozen-"));
    try {
      const archive = createTestPackageArchive([addedPath], BUNDLE_PROFILE_V14);
      await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V14);
      await assert.rejects(
        () => bundleVerifier.verifyPublisherBundle(directory),
        /outside the release allowlist|RC15 file outside its historical profile/,
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
...
test("publisher bundle v12 rejects the RC13-only prospect review inventory", async () => {
  for (const addedPath of [
    ...
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v12-frozen-"));
    try {
      const archive = createTestPackageArchive([addedPath], BUNDLE_PROFILE_V12);
      await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V12);
      await assert.rejects(
        () => bundleVerifier.verifyPublisherBundle(directory),
        /outside the release allowlist|outside its historical profile/,
...
test("publisher bundle v11 rejects the RC12-only recruitment inventory", async () => {
  for (const addedPath of [
    ...PACKED_COMPILED_ADDITIONS_V12_ONLY,
    "package/docs/publisher-recruitment.md",
    "package/schemas/launchrig-private-cohort-register-draft-result.schema.json",
    ...
    const directory = await mkdtemp(path.join(os.tmpdir(), "launchrig-bundle-v11-frozen-"));
    try {
      const archive = createTestPackageArchive([addedPath], BUNDLE_PROFILE_V11);
      await writeSyntheticBundle(directory, [], archive, BUNDLE_PROFILE_V11);
      await assert.rejects(
        () => bundleVerifier.verifyPublisherBundle(directory),
        /outside the release allowlist|outside its historical profile/,
...
test("bundle quickstart local guide links resolve inside the handoff layout", async () => {
  const quickstart = await readFile(path.join(process.cwd(), "docs", "publisher-pilot-quickstart.md"), "utf8");
  const links = [...quickstart.matchAll(/\]\(([^)]+\.md)\)/g)].map((match) => match[1]!);
  assert.ok(links.includes("publisher-recruitment.md"));
  assert.ok(links.includes("cohort-audit.md"));
  for (const link of links) {
    const resolved = path.posix.normalize(path.posix.join("docs", link));
    assert.ok(REQUIRED_PAYLOADS_V15.includes(resolved), "missing outer payload for " + link);
  }
});
