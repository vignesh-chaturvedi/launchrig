...
const MANIFEST_PATH = path.join(WORKSPACE, "fixtures", "launchrig-dapp-v0.1.0.json");
...
interface FixtureManifest {
  ...
  versionCode: number;
  network: string;
  deepLinks: { broken: string; fixed: string };
  build: {
    artifactName: string;
...
test("controlled matrix configs reference the provenance artifact", async () => {
  ...
  const [broken, fixed] = await Promise.all([
    loadConfig(path.join(WORKSPACE, "launchrig-fixture-broken.yml")),
    loadConfig(path.join(WORKSPACE, "launchrig-fixture-fixed.yml")),
  ]);
